    ...
    for (block in cor_blocks) {
      ...
      if (k_conn_pos > 0) {
        ...
        if (length(pos_candidates_idx_in_subset) > 0) {
          ...
          if (num_to_keep_pos > 0 && length(pos_subset_vals) > 0) {
            # Order all values and take the top num_to_keep_pos
            ordered_idx <- order(pos_subset_vals, decreasing = TRUE)
            top_pos_in_subset_ordered_idx <- ordered_idx[seq_len(num_to_keep_pos)]
      ...
      if (k_conn_neg > 0) {
        ...
        if (length(neg_candidates_idx_in_subset) > 0) {
          ...
          if (num_to_keep_neg > 0 && length(neg_subset_vals) > 0) {
            # Order all values and take the top num_to_keep_neg (most negative)
            ordered_idx <- order(neg_subset_vals, decreasing = FALSE)
            top_neg_in_subset_ordered_idx <- ordered_idx[seq_len(num_to_keep_neg)]
